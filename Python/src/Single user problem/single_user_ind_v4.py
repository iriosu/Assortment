...
def ComputeDistNewMatches(m,prob):
    prob_nm = {0:{0:1}}
    for nl in range(m+1): # number likes
        prob_nm[nl] = {}
        dbinom = stats.binom(nl,prob)
        for nm in range(nl+1):
            prob_nm[nl][nm] = dbinom.pmf(nm)
    return prob_nm

def ComputeProbabilities(qualities,assortments,t,theta,num_matches,potentials,CV,distr_death_matches, distr_new_matches,df):
    '''
    ...
    for s in range(len(assortments)):
        ...
        # compute prob of k users are liked
        ...
        EV[s] = 0
        for nl in range(len(S)+1): # new likes
            for nm in distr_new_matches[nl]:
                for dm in distr_death_matches[num_matches]:
                    EV[s] += (nm + df*CV[P_next,num_matches+nm-dm]) * P[s][nm] * distr_new_matches[nl][nm] * distr_death_matches[num_matches][dm]

    return U,P,EV

def SolveBackwards(n,m,T,theta,dnm,dmd,df=1):
    P = range(n)
        ...
        if t > T:
            ...
            for sP in Ps:
                for nm in range(m*(t-1)+1):
                    ...
                    Us, Pr, EV = ComputeProbabilities(Qs,As,t,theta,nm,sP,V[t+1],dnm,dmd,df)
